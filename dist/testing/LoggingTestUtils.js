...
export class MockLogGenerator {
     ...
     */
    generateLogEntries(count, baseOverrides = {}) {
...
export class MockLogger extends EventEmitter {
    ...
    log(level, message, metadata, context) {
        const entryData = {
            ...
            correlationId: (context === null || context === void 0 ? void 0 : context.correlationId) || 'test-correlation',
...
export class LoggingTestAssertions {
     ...
     */
    static assertHasLogWithMessage(logger, expectedMessage, level) {
        ...
        if (!hasMessage) {
            throw new Error(`Expected log message "${expectedMessage}" ${level ? `at level ${level}` : ''} not found. ` +
     ...
     * Assert log count
     */
    static assertLogCount(logger, expectedCount, level) {
        const actualCount = level ? logger.getLogsByLevel(level).length : logger.getLogCount();
        if (actualCount !== expectedCount) {
            throw new Error(`Expected ${expectedCount} logs${level ? ` at level ${level}` : ''}, but got ${actualCount}`);
        }
    }
    /**
     * Assert that logs are properly correlated
     */
    static assertLogsAreCorrelated(logs, correlationId) {
        ...
        if (correlatedLogs.length !== logs.length) {
            throw new Error(`Expected all ${logs.length} logs to have correlation ID ${correlationId}, ` +
     ...
     */
    static assertLogPerformance(logs, maxDuration) {
        const slowLogs = logs.filter(log => { var _a; return ((_a = log.performance) === null || _a === void 0 ? void 0 : _a.duration) && log.performance.duration > maxDuration; });
        if (slowLogs.length > 0) {
            throw new Error(`Found ${slowLogs.length} logs with duration > ${maxDuration}ms: ` +
                slowLogs.map(log => { var _a; return `${log.id} (${(_a = log.performance) === null || _a === void 0 ? void 0 : _a.duration}ms)`; }).join(', '));
        }
    }
    /**
     * Assert PII redaction
     */
    static assertPIIRedacted(logs, piiPatterns) {
        const logsWithPII = [];
        logs.forEach(log => {
        ...
        if (logsWithPII.length > 0) {
            throw new Error(`Found ${logsWithPII.length} logs with unredacted PII: ` +
                logsWithPII.map(item => `Log ${item.log.id} contains "${item.match}" (pattern: ${item.pattern})`).join(', '));
        }
    }
}
/**
...
export class LoggingPerformanceTests {
     ...
     */
    static async testLoggingThroughput(logger, messageCount, concurrency = 1) {
     ...
     */
    static async testMemoryUsage(logger, durationMs, logsPerSecond) {
...
export class LoggingIntegrationTestHelpers {
     ...
     */
    static async runMultiServiceScenario(loggers) {
        ...
        (_a = loggers.auth) === null || _a === void 0 ? void 0 : _a.info('User login initiated', { action: 'login_start' }, context);
        await this.delay(50);
        (_b = loggers.auth) === null || _b === void 0 ? void 0 : _b.info('Authentication successful', { action: 'login_success' }, context);
        await this.delay(100);
        (_c = loggers['cv-processing']) === null || _c === void 0 ? void 0 : _c.info('CV upload received', { action: 'upload_received' }, context);
        await this.delay(200);
        (_d = loggers['cv-processing']) === null || _d === void 0 ? void 0 : _d.info('CV processing started', { action: 'processing_start' }, context);
        await this.delay(1000);
        (_e = loggers['cv-processing']) === null || _e === void 0 ? void 0 : _e.info('CV analysis completed', { action: 'analysis_complete' }, context);
        await this.delay(100);
        (_f = loggers.multimedia) === null || _f === void 0 ? void 0 : _f.info('Generating podcast', { action: 'podcast_generation' }, context);
        await this.delay(500);
        (_g = loggers.premium) === null || _g === void 0 ? void 0 : _g.info('Checking subscription status', { action: 'subscription_check' }, context);
        await this.delay(50);
        const endTime = Date.now();
...
//# sourceMappingURL=LoggingTestUtils.js.map